...
- **Steam roadmap approved + progression-reward fix (2026-06-09)**:
  - Phases 0–3 of the "Lost & Found Co. feel" vertical-slice plan are implemented and live-verified across the 5 hero levels: companion bird (guide hooks implemented), story beats during play, interactive covers (`interactives[]` + `hiddenUnder`), progression rewards.
  - **Reward bug fixed**: `WinScene` granted case `hubUnlock` furniture only inside the `optionalAllFound` branch, which is always false for hero levels (they ship `optionalObjectives: []`) — so case furniture never unlocked. Grant moved into the first-time-completion block (`WinScene.js` ~53: `markCaseComplete` + `addHubUnlock` + `addOwnedFurniture`); the optional branch keeps only the bonus coins. Id namespace reconciled: `campaign.js` `hubUnlock` values changed from `furniture-books` style to the short shop ids (`books|kettle|clock|pillow|teapot`) matching `FURNITURE_ITEMS.id` in `HubScene`. Verified live end-to-end with a localStorage snapshot/restore: first-time kitchen win → owned `["kettle"]`, hubUnlocks `["kettle"]`, coins 220→340, rep 60→120, shop shows Copper Kettle owned; player save restored intact afterwards.
  - Menu wallet added: coins + reputation `createStatusPill`s top-left of `MenuScene`. `HubScene` shop label corrected to "✓ Owned" (was "✓ Placed", wrong for owned-but-unplaced case rewards). Combo audit: `registerCombo` is purely time-windowed (8s), no reset-on-miss — the planned "grace window" already exists, no change made.
  - **Phase 4 (playtest) verification complete**: forest-bookshop cover-reveal post-state confirmed live for BOTH covers (`cover-thick-book` → `bookshop-old-key` and `cover-armchair-cushion` → `bookshop-reading-glasses`: object visible, clickZone visible + input enabled, id in `openedInteractives`); non-hero regression load clean (`snowy-cabin-loft`: 10 objects, no hero controller, zero console warnings/errors); `npm run build` green (7.2s, same bundle shape).
  - **Vercel finding (this session)**: the repo auto-deploys to Vercel from GitHub (project `whimsy-hollow`, public domain `whimsy-hollow.vercel.app`), but the **production branch is still `codex/whimsy-hollow-vertical-slice`** — production serves the 2026-05-25 build (pre-crash-fix, pre-rewards), while `main` pushes only create preview deployments. Fix order matters: switch the Vercel production branch to `main` (dashboard → Settings → Git) BEFORE deleting the stale remote branch, or production deploys break. Also: `origin/main` gained a README commit (`b9c5725`, made on GitHub ~2026-06-07) that local didn't have — pulled before committing.
  - **Steam roadmap approved** (gates live in the session plan file; decisions inline here so this doc stands alone): keep AI art, $0 budget, disclosed on Steam, priced in the $2.99–4.99 value tier (comps: 100 Hidden Cats, Wind Peaks); curate the best ~20–30 non-hero scenes into 6–8 themed chapters reusing the case engine; demo-first → Next Fest → paid launch; register Steamworks early (no account yet). Tech design settled: keep Tauri + Rust `steamworks` crate via Tauri commands (Steam overlay impossible for WebView2 — out of scope), file-backed save mirror (`saveBridge.js`) + Steam Auto-Cloud (localStorage alone is purge-prone), `__WH_DEMO__` Vite-define demo flavor on a separate appid, ~19 achievements mapped to existing events, **Windows-only launch built via GitHub Actions** (Tauri can't cross-compile from macOS; Proton can't run WebView2 → Steam Deck ships "Unsupported"; macOS/Linux are phase 2). Audio/SFX license provenance is undocumented — must be audited before sale.

- **Green-screen crash fix + GitHub recovery (2026-05-28)**:
  - Fixed the intermittent "click a place/scene → stuck on a blank green screen, nothing happens" hang. Root cause: Phaser reuses the same scene instance across restarts, so on re-entry `GameScene`'s `this.ghostButton` was a **stale reference to the previous run's destroyed Text**. On **hero levels only**, `HeroLevelController.setup()` → `updateJujuHud()` → `ghostButton.setLabel()` runs inside `buildScene()` (~line 183) BEFORE `createHud()` recreates the button (~line 888), so `text.setText()` hit a destroyed Text whose canvas was null → `Cannot read properties of null (reading 'drawImage')`. First-ever entry never crashed (no stale ref yet), which is why it was intermittent ("sometimes").
  - Fix: null `this.ghostButton` in the stale-reference reset block at the top of `GameScene.buildScene()` (alongside the pre-existing `hudLayer`/`worldLayer`/`uiCam` resets — same bug class the layer reset already guards). The recreated button's default label `'✨ 0'` already matches the fresh `jujuCharges = 0` state, so no juju count is lost.
  - Defensive hardening so any future throw during scene entry can never trap the player on green again: unknown-`levelId` guards in `GameScene`/`WinScene`/`DeskScene`/`CampaignScene` (log + bounce to `MenuScene`); `GameScene.create()` and `DeskScene.create()` wrapped in try/catch that recovers to the menu and **logs the failing level id**; `LoadingScene` gained an 8s watchdog + idempotent `handoff()` + `loaderror` logging so a stalled loader can't hang. `main.js` adds a DEV-only `window.__WH_GAME__` handle (stripped from production builds).
  - Verified live (Vite dev on 127.0.0.1): all 5 hero levels (`cozy-dream-bedroom`, `forest-bookshop`, `restaurant-kitchen`, `whimsy-living-room`, `mail-garden`) survive repeated re-entry including the real `LoadingScene → GameScene` handoff; non-hero levels unaffected; zero `drawImage`/build-failed errors; `npm run build` passes (122 modules). mail-garden renders full scene + HUD (10 objects, `✨ 0`).
  - **GitHub recovery**: pushes had been silently failing because the remote `sonnymay/whimsy-hollow` was **archived (read-only)** — not deleted/merged. Unarchived it (`gh api -X PATCH repos/sonnymay/whimsy-hollow -F archived=false`), committed all previously-uncommitted local work + the crash fix as one commit (`57d4259`, 31 files, +2779/-527), renamed the branch `codex/whimsy-hollow-vertical-slice` → `main`, pushed `-u origin main`, and set `main` as the GitHub default branch. Added `.claude/` to `.gitignore`.
  - **Open items for next session**: (1) the stale remote branch `codex/whimsy-hollow-vertical-slice` still exists on origin — left in place because deletion is destructive and awaiting confirmation (`git push origin --delete codex/whimsy-hollow-vertical-slice` once confirmed). (2) The vertical-slice feature work (campaign/case system, hero-level Juju, ambient particles, reworked scenes/HUD) and the crash fix were committed **together** because they were deeply intermingled in the same files (573 changed lines in GameScene.js alone); a clean feat/fix split risked a non-building intermediate commit.
...
- **Paper Boat Rainy Window Scene (2026-05-24)**:
  ...
  - Added 10 transparent paper-boat object PNGs and 3 transparent rain-charm bonus PNGs under `public/assets/objects/` and `public/assets/bonus/`.
  - Verification for this pass: registry/progression/music check, PNG/WebP asset integrity check, `git diff --check`, and `npm run build`.

- **Wind-Up Music Box Desk Scene (2026-05-24)**:
  - Added a new non-duplicate cozy music-box repair level after `quilled-paper-blossom-desk`, with data-only registration in `src/data/levels.js` and a cozy music mapping in `src/data/music.js`.
  - Generated and normalized `public/assets/backgrounds/wind_up_music_box_desk.webp` as a 1920x1080 painterly cottage music-box worktable. The left side was visually inspected and filled with window, vines, potted plants, ribbon basket, chair, cloth, shelf, and tabletop craft props instead of empty wall beyond the narrow sidebar buffer.
  - Added 10 transparent music-box object PNGs and 3 transparent golden-note bonus PNGs under `public/assets/objects/` and `public/assets/bonus/`.
  - Verification for this pass: registry/progression/music check, PNG/WebP asset integrity check, `git diff --check`, and `npm run build`.
...
- **Painted Egg Keepsake Table Scene (2026-05-24)**:
  ...
  - Added 10 transparent painted-egg object PNGs and 3 transparent golden-egg bonus PNGs under `public/assets/objects/` and `public/assets/bonus/`.
  - Verification for this pass: registry/progression/music check, PNG/WebP asset integrity check, `git diff --check`, and `npm run build`.

- **Cinnamon Pomander Window Table Scene (2026-05-24)**:
  - Added a new non-duplicate cozy spice-craft level after `rainstick-bead-filling-table`, with data-only registration in `src/data/levels.js` and a cozy music mapping in `src/data/music.js`.
  - Generated and normalized `public/assets/backgrounds/cinnamon_pomander_window_table.webp` as a 1920x1080 painterly cottage pomander table. The left side was visually inspected and filled with window, vines, potted plants, chair, baskets, oranges, cloth, jars, and tabletop craft props instead of empty wall beyond the narrow sidebar buffer.
  - Added 10 transparent cinnamon-pomander object PNGs and 3 transparent clove-star bonus PNGs under `public/assets/objects/` and `public/assets/bonus/`.
  - Verification for this pass: registry/progression/music check, PNG/WebP asset integrity check, `git diff --check`, and `npm run build`.
...
- **Felt Star Mobile Workbench Scene (2026-05-24)**:
  ...
  - Added 10 transparent felt-mobile object PNGs and 3 transparent golden-stitch bonus PNGs under `public/assets/objects/` and `public/assets/bonus/`.
  - Verification for this pass: registry/progression/music check, PNG/WebP asset integrity check, `git diff --check`, and `npm run build`.
...
## Scene Production Update — Cloud Loom Weaving Nook

- Added `Cloud Loom Weaving Nook` with id `cloud-loom-weaving-nook`, inserted after `seashell-craft-nook` in the level registry.
- Created the painted 1920x1080 background at `public/assets/backgrounds/cloud_loom_weaving_nook.webp`.
- Created 10 transparent hidden-object PNGs under `public/assets/objects/cloud_loom_weaving_nook_*.png`.
- Created 3 transparent bonus golden-cloud PNGs under `public/assets/bonus/cloud_loom_weaving_nook_golden_cloud_*.png`.
- Wired `src/data/levels/cloud-loom-weaving-nook.js`, `src/data/levels.js`, and `src/data/music.js`.
- Background composition note: the far-left sidebar buffer is calm, but the rest of the left side is filled with a woven wall hanging, trailing plants, yarn cubbies, drawer unit, baskets, folded textiles, hanging charms, and floor/table craft details, with no large blank wall/table/floor area.

## Scene Production Update — Sunflower Pressing Table

- Added `Sunflower Pressing Table` with id `sunflower-pressing-table`, inserted after `cloud-loom-weaving-nook` in the level registry.
- Created the painted 1920x1080 background at `public/assets/backgrounds/sunflower_pressing_table.webp`.
- Created 10 transparent hidden-object PNGs under `public/assets/objects/sunflower_pressing_table_*.png`.
...
## Scene Production Update — Teacup Bonsai Bench

- Added `Teacup Bonsai Bench` with id `teacup-bonsai-bench`, inserted after `dandelion-wind-chime-porch` in the level registry.
- Created the painted 1920x1080 background at `public/assets/backgrounds/teacup_bonsai_bench.webp`.
- Created 10 transparent hidden-object PNGs under `public/assets/objects/teacup_bonsai_bench_*.png`.
...
## Scene Production Update — Cranberry Garland Hearth

- Added `Cranberry Garland Hearth` with id `cranberry-garland-hearth`, inserted after `rainy-umbrella-repair-stand` in the level registry.
- Created the painted 1920x1080 background at `public/assets/backgrounds/cranberry_garland_hearth.webp`.
- Created 10 transparent hidden-object PNGs under `public/assets/objects/cranberry_garland_hearth_*.png`.
- Created 3 transparent bonus golden-cranberry PNGs under `public/assets/bonus/cranberry_garland_hearth_golden_cranberry_*.png`.
- Wired `src/data/levels/cranberry-garland-hearth.js`, `src/data/levels.js`, and `src/data/music.js`.
- Background composition note: the far-left sidebar buffer is calm, but the rest of the left side is filled with mantel shelves, potted greenery, firewood baskets, cranberry garlands, quilt folds, stool, hearth tools, jars, table edges, and craft props, with no large blank wall/table/floor area.

## Scene Production Update — Little Library Return Cart

- Added `Little Library Return Cart` with id `little-library-return-cart`, inserted after `cranberry-garland-hearth` in the level registry.
- Created the painted 1920x1080 background at `public/assets/backgrounds/little_library_return_cart.webp`.
- Created 10 transparent hidden-object PNGs under `public/assets/objects/little_library_return_cart_*.png`.
- Created 3 transparent bonus golden-bookmark PNGs under `public/assets/bonus/little_library_return_cart_golden_bookmark_*.png`.
- Wired `src/data/levels/little-library-return-cart.js`, `src/data/levels.js`, and `src/data/music.js`.
- Background composition note: the far-left sidebar buffer is calm, but the rest of the left side is filled with trailing vines, wood shelves, wicker baskets, a return box, a stool, books, ribbons, plants, cart edges, and reading props, with no large blank wall/table/floor area.

## Scene Production Update — Cottage Key Rack Entryway

- Added `Cottage Key Rack Entryway` with id `cottage-key-rack-entryway`, inserted after `little-library-return-cart` in the level registry.
- Created the painted 1920x1080 background at `public/assets/backgrounds/cottage_key_rack_entryway.webp`.
- Created 10 transparent hidden-object PNGs under `public/assets/objects/cottage_key_rack_entryway_*.png`.
...
## Scene Production Update — Lavender Soap Stamp Sink

- Added `Lavender Soap Stamp Sink` with id `lavender-soap-stamp-sink`, inserted after `pastel-birdhouse-painting-bench` in the level registry.
- Created the painted 1920x1080 background at `public/assets/backgrounds/lavender_soap_stamp_sink.webp`.
- Created 10 transparent hidden-object PNGs under `public/assets/objects/lavender_soap_stamp_sink_*.png`.
